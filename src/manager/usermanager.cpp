...
void CUserManager::SendLoginPacket(IUser* user, const CUserCharacter& character)
{
	...
	Logger().Info("=== TEST 4: Metadata + Shop packets ===");
	
	g_PacketManager.SendUserStart(socket, user->GetID(), user->GetUsername(), character.gameName, true);
	
	Logger().Info("[TEST4] Sending Metadata packets...");
	SendMetadata(socket);
	
	// Add Shop packets
	Logger().Info("[TEST4] Sending Shop packets...");
	...
	Logger().Info("=== TEST 4 COMPLETE - Does it crash? ===");
}
